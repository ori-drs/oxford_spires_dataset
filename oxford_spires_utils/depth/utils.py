...
def load_fnt_params(
    ...
    fov_deg = 165.0  # TODO
    if depth_pose_format == "nerf":
        print("Depth Image: Using NeRF transforms.json 's Intrinsics (from colmap)")
        import json

        colmap_traj = json.load(open(depth_pose_path, "r"))
        if len(camera_topics_labelled) > 1:
            assert list(colmap_traj.keys()) == ["camera_model", "frames"]
            assert colmap_traj["camera_model"] == sensor.camera_model
            for frame in colmap_traj["frames"]:
                if frame["file_path"].split("/")[1] == camera_topics_labelled[cam_name]:
                    K, D, h, w = get_K_D_h_w_from_colmap_frame(frame, sensor.camera_model, cam_name)
                    break
        elif len(camera_topics_labelled) == 1:
            assert colmap_traj["camera_model"] == sensor.camera_model
            K, D, h, w = get_K_D_h_w_from_colmap_frame(colmap_traj, sensor.camera_model, cam_name)
        else:
            raise RuntimeError("Invalid camera_topics_labelled")
    elif depth_pose_format == "vilens_slam":
        print("Depth Image: Using Frontier_config / Kalibr for Intrinsics")
        K = sensor.get_camera(cam_name).get_K()
        D = np.array(sensor.get_camera(cam_name).extra_params)
        h = sensor.get_camera(cam_name).image_height
        w = sensor.get_camera(cam_name).image_width
        print(f"{cam_name} K: {K}, D: {D}, h: {h}, w: {w}")

    return K, D, h, w, T_base_cam, fov_deg
